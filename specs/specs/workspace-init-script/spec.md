...
Define o comportamento do `scripts/init-workspace.sh`: instalação/inicialização do OpenSpec, criação dos symlinks de specs/skills/agents, geração dos stubs `CLAUDE.md`/`AGENTS.md` na raiz do workspace e agregação de configuração de plugins, sem depender de clonagem de repositórios ou de um manifesto.

## Requirements

### Requirement: Instalação e inicialização do OpenSpec na raiz do workspace
O `scripts/init-workspace.sh` SHALL instalar e inicializar o OpenSpec para o Claude Code na raiz do `workspace` quando executado.
